/**
 * @file: WarehouseCasesTab.tsx
 * @description: Вкладка "Кейсы" с таблицей, фильтрами и функционалом управления кейсами.
...
import './WarehouseCasesTab.css';

export interface EquipmentCaseDto {
  id?: number;
  name: string;
  sku?: string;
  photoUrl?: string;
...
interface WarehouseCasesTabProps {
  cases: EquipmentCaseDto[];
  onCreateCase: () => void;
...
export function WarehouseCasesTab({
  cases,
  onCreateCase,
  onEditCase,
  onViewCase,
  loading = false,
}: WarehouseCasesTabProps) {
  ...
  const filteredCases = useMemo(() => {
    ...
    return filtered;
  }, [cases, searchQuery, statusFilter, priceRange, hideArchived]);
  ...
  const handleSelectAll = () => {
    if (selectedItems.size === paginatedCases.length) {
      setSelectedItems(new Set());
    } else {
      setSelectedItems(new Set(paginatedCases.map((item) => item.id!)));
    }
  };
  ...
  const maxPrice = useMemo(() => {
    if (cases.length === 0) return 1000000;
    return Math.max(...cases.map((c) => c.shiftPrice), 1000000);
  }, [cases]);

  return (
    <div className="warehouse-cases-tab">
      ...
      <div className="warehouse-controls">
        ...
        <div className="warehouse-controls-right">
          ...
          <button className="warehouse-button-primary" onClick={onCreateCase}>
            + Создать кейс
          </button>
        </div>
      </div>
      ...
      <div className="warehouse-summary">
        <div className="warehouse-summary-left">
          <span>Отображаются {paginatedCases.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}-{Math.min(currentPage * itemsPerPage, filteredCases.length)} из {filteredCases.length}</span>
        </div>
      ...
      <div className="warehouse-table-container">
        {loading && filteredCases.length === 0 ? (
          <div className="empty-state">Загрузка кейсов...</div>
        ) : filteredCases.length === 0 ? (
          <div className="empty-state">Кейсы не найдены</div>
        ) : (
            ...
            <thead>
              <tr>
                <th>
                  <input
                    type="checkbox"
                    checked={selectedItems.size === paginatedCases.length && paginatedCases.length > 0}
      ...
      {filteredCases.length > 0 && (
        <div className="warehouse-pagination">
          ...
          <span className="warehouse-pagination-info">
            Отображаются {paginatedCases.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}-{Math.min(currentPage * itemsPerPage, filteredCases.length)} из {filteredCases.length}
          </span>
