...
class EventsRepository {
    ...
    async getById(id) {
        const result = await database_1.pool.query('SELECT * FROM event WHERE id = $1', [id]);
    ...
    async create(payload) {
        const result = await database_1.pool.query(`INSERT INTO event (
    ...
    async update(id, payload) {
        ...
        params.push(id);
        const result = await database_1.pool.query(`UPDATE event SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, params);
        if (result.rows.length === 0) {
            return undefined;
        }
        return this.mapRowToRecord(result.rows[0]);
    ...
    async remove(id) {
        const result = await database_1.pool.query('DELETE FROM event WHERE id = $1', [id]);
